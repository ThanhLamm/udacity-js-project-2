 ...
 */
const updateApplicationState = async (element, state, newState = {}, callback = null) => {
...
const generateAppContent = (state) => {
  ...
  const galleryHtml = gallery
    ? gallery.get("photos").map((photo) => createPhotoCard(photo)).join("")
    : "";
  ...
  return `
    ...
    <main class="container py-5 light-mode">
    ...
    <footer class="py-3 text-center light-mode">
      <div class="container">
...
const createGreeting = (name) => {
  return `<h2 class="display-4">${name ? `Welcome, ${name}!` : "Hello!"}</h2>`;
};
...
const createRoverCard = (state, rover) => {
  ...
  const isLoading = Immutable.Map.isMap(selectedRover) && selectedRover.get("loading");
  const roverName = Immutable.Map.isMap(selectedRover) ? selectedRover.get("name") : null;
    ...
    <div class="col mb-4">
      <div class="card border-primary shadow-sm">
        <div class="card-body">
          ...
          <button class="btn btn-primary" onclick="showRoverDetails(${convertToJSON(state)}, ${convertToJSON(rover)})">
...
const showRoverDetails = (state, rover) => {
  const roverData = Immutable.Map.isMap(rover) ? rover : Immutable.fromJS(rover);
...
const fetchRoverList = (callback) => {
  fetch("http://localhost:3000/rovers")
    ...
    .catch((error) => console.error("Error fetching rover list:", error));
};
...
const fetchRoverPhotos = (roverName, maxDate, callback) => {
  fetch(`http://localhost:3000/rovers/${roverName}?max_date=${maxDate}`)
    .then((response) => response.json())
    .then((json) => callback(json))
    .catch((error) => console.error("Error fetching rover photos:", error));
};

// Initialize the application when the page loads.
window.addEventListener("load", () => {
  const initialState = Immutable.Map({
    user: Immutable.Map({ name: "LamNHT1" }),
    selectedRover: false,
    selectedRoverGallery: false,
  });
